...
{
    public class AppLogger : ITraceWriter
    {
        ...
        public void Error(HttpRequestMessage request, string category, TraceLevel level, Exception exception)
        {
            if (level != TraceLevel.Off)
            {
                if (exception != null)
                {
                    category = category + Environment.NewLine + "Action Parameters : " + exception.ToString();
                }
                var record = new TraceRecord(request, category, level);
                Log(record);
            }
